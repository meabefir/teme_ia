...
class Piece:
	def __init__(self, board, blocks, name):
		...
		self.bbrow1 = min([el[0] for el in blocks])
		self.bbrow2 = max([el[0] for el in blocks])
		self.bbcol1 = min([el[1] for el in blocks])
		self.bbcol2 = max([el[1] for el in blocks])
	...
	def isOOB(self):
		if self.bbrow2 >= 0 or self.bbrow1 < len(self.board) or self.bbcol2 >= 0 or self.bbcol1 < len(self.board[0]):
			return False
		return True
	...
	def __str__(self):
		...
		str_mat = [['.' for _ in range(self.bbcol2 - self.bbcol1 + 1 + 2)] for _ in range(self.bbrow2 - self.bbrow1 + 1 + 2)]
		for block in self.blocks:
		...
		for (name, piece) in nodCurent.info.items():
			for dir in dirs:
				...
				if nodCurent.contineInDrum(new_board):
					continue
				nod_nou = NodParcurgere(new_board, nodCurent, new_info, nodCurent.g + move_cost, self.calculeaza_h(nodCurent, tip_euristica), dir = dir, movedPiece = name)
